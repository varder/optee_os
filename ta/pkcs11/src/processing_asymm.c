				       ...
				       void *out_buf, uint32_t *out_sz)
{
	...
	rc = TEE_AsymmetricEncrypt(proc->tee_op_handle,
				   tee_attrs, tee_attrs_count,
				   aes_key_value, aes_key_size,
				   out_buf, &aes_wrapped_size);
	if (rc) {
		if (rc == TEE_ERROR_SHORT_BUFFER)
			*out_sz = aes_wrapped_size + data_sz + kw_semiblock_len;

		rc = tee2pkcs_error(rc);
		goto out;
	}

	if (*out_sz < aes_wrapped_size) {
					 ...
					 void **out_buf, uint32_t *out_sz)
{
	...
	*out_buf = TEE_Malloc(target_key_size, TEE_MALLOC_FILL_ZERO);
	...
	mbedtls_nist_kw_init(&kw_ctx);
	rc = TEE_AsymmetricDecrypt(proc->tee_op_handle,
				   tee_attrs, tee_attrs_count,
				   data, rsa_key_size,
				   aes_key_value, &aes_key_size);
	if (rc) {
